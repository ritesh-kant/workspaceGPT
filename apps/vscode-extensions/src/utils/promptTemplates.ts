  ...
  const contextInstruction = isGreeting
    ...
    : codebaseToolsEnabled && (broadInvestigation || writeWorkflow)
      ? (withContext
        ...
        'You can also CHANGE the workspace when the user asks for it: `edit_file` replaces exact text in a file — read_file the file first, then copy oldString character-for-character from that output, keeping its line breaks and indentation (never collapse a multi-line function onto one line, never retype code from memory); the match must be exact and unique unless replaceAll. `create_file` makes new files, `delete_file` removes them. Every write is shown to the user as a diff for approval before it is applied; a rejection returns their feedback — adjust and try again rather than repeating the same edit. Batch every change to ONE file into a single edit_file call via its `edits` array (each entry an exact oldString/newString pair, applied in order) — one call per file, never one call per change; prefer focused replacements over a full-file rewrite. When renaming or replacing something, update EVERY reference — the definition, export/module.exports lines, imports/requires, and every call site. For a named function, method, class or variable, get that list from `find_references` on its definition before editing: it returns the call sites of THAT symbol by type, including ones text search cannot tell apart (a method name other classes also have, a call through a field typed elsewhere). Then run `search_codebase` on the OLD name as a completeness check and update any remaining match that is the same symbol — hits on other symbols with the same name are expected and must stay. After edits are applied, call `get_diagnostics` to verify you introduced no compile/type errors, and fix any you did. Never edit files the user did not ask you to change.' +
        (smallModelHarness
