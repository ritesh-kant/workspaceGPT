...
export function resolveAgainstRoots(
  roots: NamedRoot[],
...
/** Adds simple singular/plural variants so a query like "filters" can still hit `cityFilter`. */
function expandTokens(tokens: string[]): string[] {
  const out = new Set(tokens);
  for (const t of tokens) {
    if (/ies$/i.test(t) && t.length >= 5) out.add(t.slice(0, -3) + 'y');
    else if (/(x|s|z|ch|sh)es$/i.test(t)) out.add(t.slice(0, -2));
    else if (/s$/i.test(t) && !/ss$/i.test(t) && t.length >= 4) out.add(t.slice(0, -1));
  }
  return [...out];
}
 ...
 * from the Dock/Finder, so a model-supplied glob like "apps/portal/**"
...
function zeroHitNote(args: SearchCodebaseArgs, triedTokens?: string[]): string {
  ...
  const globPart = args.glob
    ? ` The glob "${args.glob}" limited which files were searched — retry WITHOUT the glob, or verify the directory exists with list_directory/find_files.`
    : ' Try different keywords or synonyms, find_symbol for symbol names, or find_files for file-name patterns.';
...
async function searchCodebaseViaJsScan(
  ...
  const tokens = expandTokens(tokenize(args.query));
  const tokenPattern =
    tokens.length > 0 ? new RegExp(tokens.map(escapeRegExp).join('|'), flags) : null;
